...
def get_notes_history(db: Session = Depends(get_db), email: str = Depends(get_current_user)):
    return (
...
def delete_note(note_id: int, db: Session = Depends(get_db), email: str = Depends(get_current_user)):
    note = db.query(NoteHistory).filter(NoteHistory.id == note_id, NoteHistory.user_email == email).first()
    if not note:
